...
| Phase | 状态 | 说明 |
|---|---|---|
| 0 威胁提供者 / 分级预测 | **完成** | `predict/` + `runPilotPredictTest` (214 pass) |
| 1 扫掠碰撞 / 评分 | **完成** | `threat/` + `runPilotThreatTest` (27 pass) |
| 2 势场 + 预览 MVP | **完成** | APF + AI 试飞 + Dbg 叠加 + 死亡回放 dump |
| 3 时空搜索 | **完成(核心)** | FreeFlight 29 动作 + MaxiMin best-first；中高难符卡对比待人工 |
...
| 5 球面缝隙 | 未开始 | |
| 6 Boss 接入 | 未开始 | |
| 7 玩家 buff | **完成(实验)** | AUTO_DODGE amp0/1/2 + 三档闪避酒；平衡/PvP 钩子仍可调 |
...
- `search/`：FreeFlightModel 29 ops、SpatioTemporalSearch 节点预算
...
- 待人工：`runClient` 开 AI 试飞 ≥60s；玩家 buff 箭雨/符卡；Phase 4 测评
...
1. **速度直控模型**:自机(靶子/玩家/Boss)按"瞬时变向的速度控制"建模,不搜索加速度空间。
   动作离散化:**14 方向**(6 轴向 + 8 体对角)× 高/低速 2 档 + 原地不动 = **29 种操作**。
...
- `PilotProfile` record:全部可调参数(反应延迟 tick、搜索深度、方向数、射线数、速度档、
  死区大小、擦弹容忍……)+ 三个预设档 `NOVICE / ADEPT / LUNATIC`(对应玩家 buff 等级与 Boss 档位)。
...
- 搜索失败(全死路)时的兜底:选存活 tick 最多的分支(蓝本经验:被封时挣扎到最后一刻,
  因为符卡下一波可能开缝)。
...
- **分级设计**(Pilot Profile 映射效果等级,风格从"救场"渐进到"接管"):
  - **等级 I(救场型)**:仅紧急机动——预测到即将命中才动,MLM 式脉冲动作
    (横移/跳/卧倒)+ 冷却,玩家保有绝大部分操控感;
  - **等级 II(辅助型)**:等级 I + 势场层持续微修正(小力度合成进玩家速度,
    带死区,玩家主动输入优先);
  - **等级 III(接管型)**:完整 Pilot(含搜索层),地面用 GroundedModel,
    飞行状态(创造飞行/鞘翅等)切 FreeFlightModel。
...
- **验收**:创造模式给自己上效果:等级 I 在骷髅箭雨中能救场且不干扰正常走位;
  等级 III 站进测试符卡能被"带着躲";效果结束干净移除,无残留速度修正。
...
7. 遇到 C2 checklist 过不了又觉得可惜的 Mover,落 T3 即可,不要发明克隆方案;
   Phase 7 的获得渠道/平衡钩子是仅剩的待用户拍板事项,动手前先问。
