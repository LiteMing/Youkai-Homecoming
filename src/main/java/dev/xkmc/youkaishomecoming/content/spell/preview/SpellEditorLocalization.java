...
public final class SpellEditorLocalization {
	...
	private static final Map<String, String> ZH = Map.ofEntries(
			...
			Map.entry("Bounce Source", "反弹源弹幕"),
			Map.entry("Continue Source", "保留源弹幕"),
			Map.entry("Expire Source", "终结源弹幕"),
			Map.entry("Discard Source", "销毁源弹幕"),
			Map.entry("bounce_source", "反弹源弹幕"),
			Map.entry("continue_source", "保留源弹幕"),
			Map.entry("expire_source", "终结源弹幕"),
			Map.entry("discard_source", "销毁源弹幕"),
			Map.entry("Delay Ticks", "延迟刻数"),
			Map.entry("Max Bounces", "最大反弹次数"),
			Map.entry("Speed Decay", "速度衰减"),
