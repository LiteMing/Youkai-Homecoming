...
public final class ModelPresentationTest {
	...
	private static void signalContracts() {
		...
		equal("combat starts on server edge", combat.combatAt(), 100L);
		...
		equal("movement transition leaves combat clock", fly.combatAt(), 100L);
		...
		equal("re-enter combat is a new event", again.combatSequence(), 2L);
		var hurt = again.hurt(141).hurt(141);
		equal("same-tick hits each have a sequence", hurt.hurtSequence(), 2L);
